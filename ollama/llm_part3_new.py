...
JSONL_PATH = "C:/Users/Fyn/Desktop/rag/json/toeic_part3_db_ready.jsonl"
...
# 查詢語意相關的範例題組與情境文字
query_embedding = embedding_model.encode("Part 3 TOEIC listening conversation with 3 questions").tolist()
...
# 🔧 產生 prompt（符合資料表格式）
prompt = f"""
你是一位專業 TOEIC 出題專家。請根據下列測驗情境與範例題目，生成一組新的 TOEIC Part 3 聽力題組，包含：

1. 一筆 listening_material（不含 ID）
2. 三筆 question（不含 ID 與 material_id）
內容必須全為英文。
對話總長度約為80~120字，約3~5句，單句平均字數約15-25字。
常見的場景有這些:
· 同事對話
· 家人朋友對話
· 購物
...
{{
  "listening_material": {{
    "audio_url": "音檔 先留空",
    "transcript": "雙人對話，對話稿開頭一定要有Questions 1 through 3. Refer to the following conversation.直接接對話稿（\\n 表示換行）",
    "accent": "口音 先留空",
    "topic": "{scenario_text}",
    "speaker_count": "說話人數",
